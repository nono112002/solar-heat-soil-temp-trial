...
## ⚠ ラベルと実際の深さのずれ（2026-07-28 時点）

**ラベル名は変更していない**。Picoのファームを農場でしか書き換えられず、
書き換えるまで新旧ラベルが混在してグラフの系列が割れるため。
実態とのずれは HP ダッシュボードの表示側で補正している。

| DBラベル | 実際 | 対象 |
|---|---|---|
| `S1_center_10cm` | 中央 **15cm** | 全ゾーン（過去データも同じ） |
| `S4_edge_10cm` | 端部 **15cm** | 全ゾーン（過去データも同じ） |
| `S7_outdoor` | 土壌 **5cm**（2026-07-28 08:50 JST〜） | 区A・区Bのみ。区Cは外気温のまま |

- 5cm は BLOF公式の基準深さだが、途中からの計測で条件が揃わないため **参考値扱い**。積算温度の計算には含めない。
- 表示補正は `portfolio/hugo/content/projects/solar-heat-data/index.md` の `LABEL_NAMES` / `labelName()` / `SOIL_5CM_ZONES`。
- Grafana は補正していないため、パネル名・列名は `10cm` 表記のまま（実際は15cm）。
- 次回農場でファームを書き換える際にラベル自体を改名するなら、DBの既存レコードも同時に UPDATE すること。
